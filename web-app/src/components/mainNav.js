    ...
    return (
      <div className="mb-1">
        <Navbar color="dark" dark expand="md">
          <Container>
            <NavbarBrand href="/">Aspire App</NavbarBrand>
    ...
    <Nav className="ml-auto" navbar>
      {navLinkList.map((navItem, index) =>
        ...
        (navItem.Auth === "Normal" || !navItem.Auth) ? (
          <NavItem key={index}>
            {navItem.onClick ? (
              <BNavLink onClick={navItem.onClick} className="nav-link">
                {navItem.name}
              </BNavLink>
            ) : (
              <NavLink exact className="nav-link" to={navItem.path}>
                {navItem.name}
              </NavLink>
            )}
          </NavItem>
